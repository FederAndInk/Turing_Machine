 ...
 * CONTENT 
 *
 *   Extension of the String module with additional functions
 *
 * USAGE
 *
 *   Require    :  MyList.ml
 *   Compilation:  ocamlc MyList.ml MyString.ml
 *   Interpreter:  ledit ocaml MyList.cmo MyString.cmo
 *)
...
let (char_list_to_string: char list -> string) = fun charS ->
  ...
  begin
    MyList.iter_counter 0 (fun i c -> String.set string i c) charS ;
    string
  end
...
let rev s =
  ...
  if l = 0 then s
    ...
    begin
      ...
      for i = 0 to last do
        String.set r (last-i) (String.unsafe_get s i)
      done ;
      r
    end
...
let map f s =
  ...
  if l = 0 then s
    ...
    begin
      let r = String.create l in
      for i = 0 to l - 1 do
        String.set r i (f (String.unsafe_get s i))
      done ;
      r
    end
